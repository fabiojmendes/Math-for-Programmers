...
def segments(obj):
    return [(obj[i], obj[(i+1) % len(obj)]) for i in range(len(obj))]
...
spaceship = [rotate2d(pi/2, v) for v in spaceship]
spaceship = translate((2, 3), spaceship)
...
intersections = [[seg1, seg2]
                 for seg1 in segments(asteroid)
                 for seg2 in segments(spaceship)
                 if segments_intersect(seg1, seg2)]

# intersections = [item for inter in intersections
#                  for item in inter]
...
draw(
    Points(*spaceship),
    Polygon(*spaceship, color=blue, fill=blue),

    Points(*asteroid),
    Polygon(*asteroid, color=green, fill=green),
    *[Segment(*seg, color=red) for item in intersections for seg in item],
)
